...
class RideSaga {
  ...
  transition(rideId, newState, extra = {}) {
    ...
    if (!allowed.includes(newState)) {
      console.warn(`[SAGA] Transição inválida: ${ride.state} -> ${newState}`);
      return null;
    }

    const clock = getClock(config.serviceId);
    const event = clock.tick(`ride.${newState}`, { rideId, ...extra });

    ride.state = newState;
    Object.assign(ride, extra);
    ride.history.push({ state: newState, ts: event.ts, at: Date.now(), ...extra });

    metrics.rideStateTransitions.inc({ state: newState });
    return ride;
  }
