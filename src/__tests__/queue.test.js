...
jest.mock('../drivers/driver-registry', () => ({
  driverRegistry: {
    availableCount: jest.fn(() => 5),
    snapshot: jest.fn(() => ({ total: 10, available: 5, busy: 5 })),
    hasAvailable: jest.fn(() => true),
  },
}));
...
jest.mock('../core/core-client', () => ({
  coreClient: {
    ...
    cbSnapshot: jest.fn(() => ({ state: 'CLOSED', failureCount: 0 })),
  },
}));
...
describe('Ride Queue — Pool de Corridas', () => {
  ...
  test('enfileira corrida com sucesso', () => {
    const q = new RideQueue(50);
    const ride = { rideId: 'ride-1', passengerId: 'p1', origin: 'A', destination: 'B', state: 'request' };
  ...
  test('rejeita enfileiramento quando fila cheia', () => {
    ...
    const ride1 = { rideId: 'ride-1', passengerId: 'p1', origin: 'A', destination: 'B', state: 'request' };
    const ride2 = { rideId: 'ride-2', passengerId: 'p2', origin: 'A', destination: 'B', state: 'request' };
    const ride3 = { rideId: 'ride-3', passengerId: 'p3', origin: 'A', destination: 'B', state: 'request' };
  ...
  test('peek retorna próxima sem remover', () => {
    ...
    expect(q.size()).toBe(2); // não removeu
  });
  ...
  test('snapshot registra métricas corretas', () => {
    ...
    expect(s2.peakSize).toBe(2); // mantém pico
...
describe('Overflow Policy — Decisão de Congestionamento', () => {
  ...
  test('retorna "delegate" quando Core está disponível e há congestionamento', () => {
    ...
    driverRegistry.snapshot.mockReturnValue({ total: 10, available: 1 });
    coreClient.isAvailable.mockReturnValue(true);

    const policy = new OverflowPolicy();
    const decision = policy.getDecision({}, { isFull: false, currentSize: 0 });
  ...
  test('retorna "reject" quando fila está cheia', () => {
    const policy = new OverflowPolicy();
    const decision = policy.getDecision({}, { isFull: true });
  ...
  test('isOverloaded retorna true quando congestionado', () => {
    ...
    const policy = new OverflowPolicy();
    expect(policy.isOverloaded()).toBe(true);
  });
  ...
  test('isOverloaded retorna false quando há capacidade', () => {
    ...
    const policy = new OverflowPolicy();
    expect(policy.isOverloaded()).toBe(false);
  });

  test('canAcceptLocal retorna true com motoristas disponíveis', () => {
    driverRegistry.availableCount.mockReturnValue(3);
    driverRegistry.snapshot.mockReturnValue({ total: 10, available: 3 });

    const policy = new OverflowPolicy();
    expect(policy.canAcceptLocal()).toBe(true);
  });

  test('canAcceptLocal retorna false sem motoristas', () => {
    driverRegistry.availableCount.mockReturnValue(0);
    driverRegistry.snapshot.mockReturnValue({ total: 10, available: 0 });

    const policy = new OverflowPolicy();
    expect(policy.canAcceptLocal()).toBe(false);
  });
  ...
  test('getMetrics retorna estado completo da política', () => {
    driverRegistry.availableCount.mockReturnValue(3);
    driverRegistry.snapshot.mockReturnValue({ total: 10, available: 3, busy: 7 });
    coreClient.isAvailable.mockReturnValue(true);
    coreClient.cbSnapshot.mockReturnValue({ state: 'CLOSED', failureCount: 0 });
    ...
    expect(metrics.policy.canAcceptLocal).toBe(true);
    expect(metrics.policy.canDelegate).toBe(true);
  });
