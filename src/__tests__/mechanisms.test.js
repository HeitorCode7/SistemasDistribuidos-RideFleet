// src/__tests__/mechanisms.test.js
// Testes dos 5 mecanismos de SD
...
describe('Req 1 — Distributed Lock', () => {
  ...
  beforeEach(() => {
    jest.resetModules();
    ({ DistributedLockManager } = require('../locks/distributed-lock'));
  });

  test('adquire lock com sucesso quando livre', () => {
    const mgr = new DistributedLockManager(1000);
    const result = mgr.acquire('ride-1', 'service-a');
  ...
  test('bloqueia segundo solicitante (contenção)', () => {
    ...
    const result = mgr.acquire('ride-1', 'service-b');
  ...
  test('libera lock corretamente', () => {
    const mgr = new DistributedLockManager(1000);
    mgr.acquire('ride-1', 'service-a');
    const released = mgr.release('ride-1', 'service-a');
    expect(released).toBe(true);
    const r2 = mgr.acquire('ride-1', 'service-b');
  ...
  test('não libera lock de outro dono', () => {
    const mgr = new DistributedLockManager(1000);
    mgr.acquire('ride-1', 'service-a');
    const released = mgr.release('ride-1', 'service-b');
  ...
  test('lock expira após TTL', done => {
    ...
    setTimeout(() => {
      const r2 = mgr.acquire('ride-ttl', 'service-b');
      expect(r2.acquired).toBe(true);
      done();
    }, 200);
  });
...
describe('Req 2 — Saga / Commit Distribuído', () => {
  ...
  test('cria corrida no estado REQUEST', () => {
    ...
    const ride = saga.create({ passengerId: 'p1', origin: 'A', destination: 'B' });
  ...
  test('transição válida REQUEST -> MATCH', () => {
    const saga = new RideSaga();
    const ride = saga.create({ passengerId: 'p1', origin: 'A', destination: 'B' });
    const updated = saga.transition(ride.rideId, RIDE_STATE.MATCH);
    expect(updated.state).toBe(RIDE_STATE.MATCH);
  });

  test('transição inválida é rejeitada (COMPLETE -> REQUEST)', () => {
    const saga = new RideSaga();
    const ride = saga.create({ passengerId: 'p1', origin: 'A', destination: 'B' });
    saga.transition(ride.rideId, RIDE_STATE.MATCH);
    saga.transition(ride.rideId, RIDE_STATE.CONFIRM);
    saga.transition(ride.rideId, RIDE_STATE.IN_TRANSIT);
    saga.transition(ride.rideId, RIDE_STATE.COMPLETE);
    const invalid = saga.transition(ride.rideId, RIDE_STATE.REQUEST);
  ...
  test('compensação leva ao estado COMPENSATING', () => {
    ...
    const ride = saga.create({ passengerId: 'p1', origin: 'A', destination: 'B' });
    saga.transition(ride.rideId, RIDE_STATE.MATCH);
    const compensated = saga.compensate(ride.rideId, 'partner_failed');
    expect(compensated.state).toBe(RIDE_STATE.COMPENSATING);
  });

  test('histórico registra todas as transições', () => {
    const saga = new RideSaga();
    const ride = saga.create({ passengerId: 'p1', origin: 'A', destination: 'B' });
    saga.transition(ride.rideId, RIDE_STATE.MATCH);
    saga.transition(ride.rideId, RIDE_STATE.CONFIRM);
    const r = saga.get(ride.rideId);
    expect(r.history.length).toBe(3); // REQUEST + MATCH + CONFIRM
  });
});
...
describe('Req 4 — Circuit Breaker', () => {
  let CircuitBreaker, STATE;
  beforeEach(() => {
    jest.resetModules();
    ({ CircuitBreaker, STATE } = require('../circuit-breaker/circuit-breaker'));
  });

  test('inicia no estado CLOSED', () => {
    const cb = new CircuitBreaker('partner-x', { failureThreshold: 3 });
  ...
  test('abre após atingir threshold de falhas', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      try { await cb.call(failFn); } catch (_) {}
    }
    expect(cb.state).toBe(STATE.OPEN);
  });

  test('bloqueia chamadas quando OPEN', async () => {
    const cb = new CircuitBreaker('partner-x', { failureThreshold: 1 });
    try { await cb.call(() => Promise.reject(new Error('fail'))); } catch (_) {}
    await expect(cb.call(() => Promise.resolve('ok'))).rejects.toThrow('CircuitBreaker OPEN');
  });

  test('fecha após sucesso em HALF_OPEN', async () => {
    const cb = new CircuitBreaker('partner-x', {
      failureThreshold: 1,
...
describe('Req 5 — Relógio de Lamport', () => {
  ...
  beforeEach(() => {
    jest.resetModules();
    ({ LamportClock } = require('../logical-clock/lamport-clock'));
  });

  test('incrementa a cada evento local', () => {
    const clock = new LamportClock('svc-a');
    const e1 = clock.tick('event.a');
    const e2 = clock.tick('event.b');
    expect(e2.ts).toBeGreaterThan(e1.ts);
  });

  test('aplica regra max+1 ao receber evento externo', () => {
    const clock = new LamportClock('svc-a');
    clock.tick('local'); // ts = 1
    const received = clock.receive(10, 'remote.event');
    expect(received.ts).toBe(11); // max(1, 10) + 1
  });

  test('relógio remoto maior prevalece', () => {
    const clockA = new LamportClock('svc-a');
    const clockB = new LamportClock('svc-b');

    clockA.tick('a1'); // A=1
    clockA.tick('a2'); // A=2
    clockB.tick('b1'); // B=1

    const recv = clockB.receive(clockA.now(), 'from-a');
    expect(recv.ts).toBe(3);
  });

  test('log causal filtrado por rideId preserva ordem', () => {
    const clock = new LamportClock('svc-a');
    clock.tick('ride.created',  { rideId: 'r1' });
    clock.tick('other.event',   { rideId: 'r2' });
    clock.tick('ride.matched',  { rideId: 'r1' });

    const log = clock.getCausalLog('r1');
